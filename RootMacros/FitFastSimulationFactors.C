// Fit fast simulation parametrization parameters to inclusive efficiency
...
Int_t FitFastSimulationFactors(TString effFile, TString outputfile, TString parameters = "") {
  ...
  for (Int_t species=0;species < AliPID::kSPECIES;species++) {
    for (Int_t charge=0;charge<2;charge++) {
      polynoms[species][charge] = new AliPieceWisePoly(parts[species][charge], cuts[species][charge], nparameters[species][charge], 0, 50, 0x0, smoothType);
    }
  }                                                          
