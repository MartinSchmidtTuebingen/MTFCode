...
void sysErrorsPythiaFastJet(TString fileDir = "files", TString saveDir = "files"){
    ...
    for (Int_t sp=0; sp<nSpecies; sp++) {
      ...
      for(Int_t i=0; i<nJetPtBins; i++){
        ...
        for (Int_t mode=0;mode<nModes;++mode) {
          ...
          TString strTit(Form("corrFac%s_%03d_%03d_%s_%d",modeString[mode].Data(), varPercent[2*var], varPercent[2*var + 1], strSp[sp].Data(), i));
    ...
    for(Int_t i=0; i<nJetPtBins; i++) {
      for (Int_t mode=0; mode<nModes; mode++) {
        ...
        for (Int_t var=0; var<nVar; var++) {
          ...
          TString strTitSys(Form("corrFac%sSys%s%s_%d", modeString[mode].Data(), nameVar[var].Data(), strSp[sp].Data(),i));
		...
		for (Int_t sp=0; sp<nSpecies; sp++) {
			for(Int_t i=0; i<nJetPtBins; i++){
				for (Int_t mode=0;mode<nModes;++mode) {
					...
					TString strTit(Form("corrFac%s_%s_%s_%d", modeString[mode].Data(), ffModeString[ffMode].Data(), strSp[sp].Data(), i));
